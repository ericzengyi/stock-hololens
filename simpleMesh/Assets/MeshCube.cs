...
public class MeshCube {
    public static void MoveCube(ref Vector3[] mesh_vertices, ref int[] mesh_triangles, ref Vector3[] mesh_normals, ref Vector2[] mesh_uv,
        ...
        for (int i = 0; i < 24; i++)
        {
            mesh_vertices[ i] = vertices[i];//cubeIndex * 24 +
        }

        for (int i = 0; i < 24; i++)
        {
            mesh_normals[i] = normals[i];
        }
        for (int i = 0; i < 24; i++)
        {
            mesh_uv[i] = uv[i];
        ...
        for (int i = 0; i < 36; i++)
        {
            mesh_triangles[i] = cubeIndex * 24 + triangles[i];
        }
    ...
    public static void AddCube( ref Vector3[] mesh_vertices, ref int[] mesh_triangles, ref Vector3[] mesh_normals, ref Vector2[] mesh_uv, 
	                           int cubeIndex, Vector3 positionOffset, float size ) {
	...
	for( int i=0; i < 24; i++ ) {
			mesh_vertices[ cubeIndex * 24 + i ] = vertices[i];
	}

	for( int i=0; i < 24; i++ ) {
		mesh_normals[ cubeIndex * 24 + i ] = normals[i];
	}
	for( int i=0; i < 24; i++ ) {
		mesh_uv[ cubeIndex * 24 + i ] = uv[i];
	}
	for( int i=0; i < 36; i++ ) {
		mesh_triangles[ cubeIndex * 36 + i ] = cubeIndex * 24 + triangles[i];
	}
	//mesh_RecalculateBounds();
	//mesh_Optimize();
